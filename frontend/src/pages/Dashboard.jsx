...
const Dashboard = () => {
  ...
  const validate = () => {
    ...
    if (!formData.workLifeBalance) errors.workLifeBalance = "Select work-life balance";
    ...
    if (!formData.remoteWork) errors.remoteWork = "Please select remote work option";
  ...
  const formFields = [
    ...
    { name: "monthlyIncome", label: "Monthly Income ($)", icon: DollarSign, type: "number" },
    { name: "workLifeBalance", label: "Work-Life Balance", icon: Clock, type: "select", options: ["Poor", "Fair", "Good", "Excellent"] },
    { name: "numberOfPromotions", label: "Number of Promotions", icon: TrendingUp, type: "number" },
    { name: "distanceFromHome", label: "Distance from Home (km)", icon: MapPin, type: "number" },
    { name: "educationLevel", label: "Education Level", icon: GraduationCap, type: "select", options: ["Associate Degree", "High School", "Bachelor's Degree", "Master's Degree", "PhD"] },
    { name: "maritalStatus", label: "Marital Status", icon: Heart, type: "select", options: ["Single", "Married", "Divorced"] },
    { name: "jobLevel", label: "Job Level", icon: Briefcase, type: "select", options: ["Entry", "Mid", "Senior", "Executive"] },
    { name: "companySize", label: "Company Size", icon: Building, type: "select", options: ["Small", "Medium", "Large"] },
    { name: "remoteWork", label: "Remote Work", icon: Wifi, type: "select", options: ["Yes", "No"] },
  ];
  ...
  return (
    <div className="max-w-4xl mx-auto mt-8 p-6 bg-white rounded-xl shadow-md">
      <h1 className="text-2xl font-bold mb-6 text-center text-gray-800">Employee Attrition Prediction</h1>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div key={name} className="flex flex-col">
            ...
            {type === "select" ? (
              <select
                ...
                className={`px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all ${formErrors[name] ? "border-red-500" : "border-gray-300"}`}
              >
        ...
        <div className="md:col-span-2 flex justify-between gap-4 mt-4">
          <button type="submit" disabled={loading} className={`w-full ${loading ? "bg-blue-400 cursor-wait" : "bg-blue-600 hover:bg-blue-700"} text-white font-medium py-2 px-4 rounded-lg transition-all`}>
            {loading ? "Predicting..." : "Predict"}
          </button>
          <button type="button" onClick={handleReset} className="w-full bg-gray-300 hover:bg-gray-400 text-gray-800 font-medium py-2 px-4 rounded-lg transition-all">
            Reset
          </button>
        </div>
      </form>
      ...
      <div ref={predictionRef}>
        {prediction !== null && (
          <div className={`mt-8 mx-auto w-full p-6 rounded-xl shadow-lg border-l-4 ${prediction === 0 ? "border-red-600 bg-red-50" : "border-green-600 bg-green-50"}`}>
            <div className="flex items-center justify-between">
              <h2 className={`text-lg font-bold ${prediction === 0 ? "text-red-700" : "text-green-700"}`}>{prediction === 0 ? "Attrition Risk Detected" : "No Attrition Risk"}</h2>
            ...
            <div className="mt-4">
              <div className="w-full bg-gray-200 rounded-full h-4 overflow-hidden">
                <div
                  className={`h-4 ${prediction === 0 ? "bg-red-600" : "bg-green-600"}`}
                  style={{ width: `${probability !== null ? ((1-probability) * 100).toFixed(2) : 0}%`, transition: "width 400ms ease" }}
                />
              </div>
              <div className="mt-2 text-sm text-gray-700">
                {probability !== null ? `${((1-probability) * 100).toFixed(2)}%` : "N/A"}
              </div>
            </div>

            <p className="mt-4 text-sm text-gray-600">
              {prediction === 0 ? "This employee may be at risk of attrition. Consider preventative action." : "This employee shows no current signs of attrition risk."}
            </p>
          ...
          <div className="grid gap-3">
            {history.map((h) => (
              <div key={h.ts} className="p-3 bg-white rounded border">
                ...
                <div className="text-sm mt-1">Prediction: <strong className={h.prediction === 0 ? "text-red-600" : "text-green-600"}>{h.prediction === 0 ? "Attrition" : "No Attrition"}</strong> — {h.probability !== null ? `${((1 - h.probability)*100).toFixed(2)}%` : "N/A"}</div>
                <details className="mt-2 text-xs text-gray-600">
                  <summary className="cursor-pointer">View input</summary>
                  <pre className="mt-2 text-xs bg-gray-100 p-2 rounded">{JSON.stringify(h.input, null, 2)}</pre>
                </details>
              </div>
            ))}
          </div>
